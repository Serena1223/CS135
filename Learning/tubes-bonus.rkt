...
(define (next-games gm)
  (local
    [(define (rm-list tubes rev-tubes rm-ind colour tube i log)
       (cond
         [(empty? tubes) log]
         [(= rm-ind i)
          (rm-list (rest tubes) rev-tubes rm-ind colour tube (add1 i) log)]
         [(< (length (first tubes)) (game-tubesize gm))
          (cons (make-game (game-tubesize gm)
                           (game-maxcolours gm)
                           (get-game rev-tubes rm-ind tube i (cons colour (first tubes)) (sub1 (length (game-tubes gm)))))
                (rm-list (rest tubes) rev-tubes rm-ind colour tube (add1 i) log))]
         [else (rm-list (rest tubes) rev-tubes rm-ind colour tube (add1 i) log)]))

     (define (get-game rev-tubes rm-ind rm-tube add-ind add-tube i)
       (cond
         [(empty? rev-tubes) empty]
         [(= i rm-ind) (cons rm-tube (get-game (rest rev-tubes) rm-ind rm-tube add-ind add-tube (sub1 i)))]
         [(= i add-ind) (cons add-tube (get-game (rest rev-tubes) rm-ind rm-tube add-ind add-tube (sub1 i)))]
         [else (cons (first rev-tubes) (get-game (rest rev-tubes) rm-ind rm-tube add-ind add-tube (sub1 i)))]))
         
     (define (get-next tubes i)
       (cond
         [(empty? tubes) empty]
         [(> (length (first tubes)) 0)
          (rm-list (game-tubes gm) (reverse (game-tubes gm)) i (first (first tubes))
                   (rest (first tubes))
                   0
                   (get-next (rest tubes) (add1 i)))]
         [else (get-next (rest tubes) (add1 i))]))]
    (get-next (game-tubes gm) 0)))
